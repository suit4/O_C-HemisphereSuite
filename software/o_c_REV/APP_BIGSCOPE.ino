...
public:
    ...
    {
        if (sample_ticks < 32)
            sample_ticks += direction;
        else
            sample_ticks += direction * 10;
        sample_ticks = constrain(sample_ticks, 2, 64000);
...
private:
    ...
    void DrawGrid()
    {
        // screen size is 128 x 64 (0-127 / 0-63)
        const int minX = 0;
        const int minY = 16;
        const int hLines = 5;
        const int vLines = 9;
        const int hDist = int(floor((128 - minX) / (vLines - 1)));
        const int vDist = int(floor((64 - minY) / (hLines - 1)));

        // vertical lines
        for (int i = 0; i < vLines; i++)
        {
            int x = min(minX + hDist * i, 127);
            gfxDottedLine(x, minY, x, 63, 4);
        }
        // horizontal lines
        for (int i = 0; i < hLines; i++)
        {
            int y = min(minY + vDist * i, 63);
            gfxDottedLine(minX, y, 127, y, 4);
        }
    }

    void DrawTicks()
    {
        gfxPrint(0, 48, sample_ticks);
    }
...
void BigScope_handleButtonEvent(const UI::Event &event)
{
    ...
    if (event.control == OC::CONTROL_BUTTON_L)
    {
        if (event.type == UI::EVENT_BUTTON_LONG_PRESS)
            BigScope_instance.OnLeftButtonLongPress();
        else
            BigScope_instance.OnLeftButtonPress();
    }
    ...
    if (event.control == OC::CONTROL_BUTTON_UP)
        BigScope_instance.OnUpButtonPress();

    // For down button, handle press and long press
    if (event.control == OC::CONTROL_BUTTON_DOWN)
    {
        if (event.type == UI::EVENT_BUTTON_PRESS)
            BigScope_instance.OnDownButtonPress();
        if (event.type == UI::EVENT_BUTTON_LONG_PRESS)
            BigScope_instance.OnDownButtonLongPress();
    }
}
...
void BigScope_handleEncoderEvent(const UI::Event &event)
{
    ...
    if (event.control == OC::CONTROL_ENCODER_L)
        BigScope_instance.OnLeftEncoderMove(event.value);

    // Right encoder turned
    if (event.control == OC::CONTROL_ENCODER_R)
        BigScope_instance.OnRightEncoderMove(event.value);
}
